...
import CopyButton from "./TransCopyButton";
import { motion } from "framer-motion";
import TransactionsMedia from "./TransactionsMedia";
import type { Meme } from "@/Types";
import Image from "next/image";
import Currencylogo from "@/public/assets/icons/CurrencyLogo";
// import chonklogo from "@/assets/icons/chonklogo.png";
...
const TransactionData: React.FC<TransactionDataProps> = ({
  ...
  const { solPrice, chonkPrice, chonkerPrice } = prices;
  ...
  useEffect(() => {
    ...
    const fetchTransactions = async () => {
      setIsLoading(true);
      try {
        const response = await fetch(`/api/transactions-data?memeId=${docId}`);
        ...
        setMemeData(data.meme as Meme);
        setTipsData(data.tips);

        // Compute total USD earnings with current live prices
        const totalUsd = data.tips.reduce((sum: number, tip: Tip) => {
  ...
  // Share button with dynamic icon
  const ShareButton = () => (
  ...
  return (
    <div className="pt-28 px-4 w-full flex justify-center">
      <div className="relative border border-[#152D2D] py-6 w-full max-w-7xl 2xl:max-w-[1600px] 2xl:py-6 mx-4 bg-[#0C1219] rounded-lg max-h-[90vh] overflow-y-auto">
        ...
        <div
          className={`px-6 py- ${
            tipsData.length > 0 && "md:flex gap-10 flex-nowrap "
          }`}
        >
          {isLoading ? (
            ...
            <div className="text-center text-gray-600 py-10 text-sm md:text-base">
              No meme transactions available
            </div>
          ) : (
            <>
              <div className="hidden md:flex font-Roboto flex-col w-[30%] gap-0">
                ...
                {/* Earnings container with fixed width */}
              ...
              <div className="w-fit overflow-x-auto 2xl:overflow-x-visible">
                <div className="max-h-[600px] overflow-y-auto overflow-x-auto shadow-[4px_4px_7px_rgba(0,0,0,0.4)]">
                  <table className="min-w-full overflow-hidden">
                    ...
                    <tbody className="bg-black-500 bg-opacity-40 rounded-3px">
                      {tipsData.map((tip, index) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="px-4 py-3 text-[14px] 2xl:text-[18px] cursor-pointer text-[#C3C8CC] hover:text-[#ffffff]">
