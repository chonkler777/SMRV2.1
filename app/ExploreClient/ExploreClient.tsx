...
import ExploreSortSkeleton from "@/components/ExploreSortSkeleton";
...
export default function ExploreMemeGrid({
  ...
  const [isShuffling, setIsShuffling] = useState(false); // ✅ New state for showing skeleton during shuffle
  ...
  // ✅ Updated handler - show skeleton when random is clicked
  ...
  // ✅ Hide skeleton when data is ready for random mode
  useEffect(() => {
    if (sortMode === "random" && !isLoadingMemes && typedPaginatedData?.pages?.length) {
  ...
  }, [sortMode, isLoadingMemes, typedPaginatedData]);
  ...
  return (
    <div className="space-y-6 pt-18 lg:pt-24">
      <div className="fixed top-[77px] lg:ml-6 z-40 -mt-[1px] left-[43px] 2xl:left-[46px] right-0 flex px-6 justify-center lg:justify-between items-center pb-1 pointer-events-none">
        <div className="hidden lg:flex text-[18px] pl-6 space-x-3 text-[#] pointer-events-auto">
          <button
            onClick={() => handleSortChange("hot")}
            disabled={isSwitchingMode || isShuffling}
            className={`${
              sortMode === "hot"
              ...
              isSwitchingMode || isShuffling ? "opacity-50 cursor-not-allowed" : ""
      ...
      {/* ✅ Show skeleton when switching modes OR shuffling, pass isShuffling prop */}
      {(isSwitchingMode || isShuffling) && <ExploreSortSkeleton isShuffling={isShuffling} />}

      {/* ✅ Only show memes when NOT switching and NOT shuffling */}
      {!isSwitchingMode && !isShuffling && !isInitialSearch && memesToDisplay.length > 0 && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 px-4 lg:px-12 gap-12 pt-8">
          {memesToDisplay.map((meme, index) => {
            ...
            return (
              // ✅ Removed motion.div and animation - now just a regular div
              <div
      ...
      {(((isLoading || isInitialSearch) && !isSwitchingMode && !isShuffling) ||
        isFetchingNextPage ||
      ...
      {!displayHasMore && memesToDisplay.length > 0 && !isSwitchingMode && !isShuffling && (
